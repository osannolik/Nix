    ...
    PinCS: OutputPin<Error = PinE>,
{
    ...
    pub fn read_time<Spi, SpiE>(&mut self, spi: &mut Spi) -> Time<Bcd>
    where
        SpiE: Debug,
        Spi: Transfer<u8, Error = SpiE> + Write<u8, Error = SpiE>,
    {
        ...
        Time::new(
            Bcd::new(time_data[0]),
            Bcd::new(time_data[1]),
            Bcd::new(time_data[2] & RegisterBits::MASK_HOURS),
        )
    }

    pub fn write_time<Spi, SpiE>(&mut self, time: &Time<Bcd>, spi: &mut Spi)
    where
        SpiE: Debug,
        Spi: Transfer<u8, Error = SpiE> + Write<u8, Error = SpiE>,
    {
        let mut time_data = [
            time.seconds.value(),
            time.minutes.value(),
            time.hours.value() & RegisterBits::MASK_HOURS,
        ];
